...
| Lekérdezés részletes adatokkal (`query` `detailed:true`) | `POST /query` | ⬜ szándékosan kihagyva |

**A két utolsó sor szándékosan kihagyott, nem elfelejtett (Task 13, 2026-08-30).** Mindkét SimplePay-kapcsoló extra mezőket hoz a válaszba, amiket a csomag jelenleg nem tud kiolvasni — bekapcsolásuk így néma ígéret lett volna. A `refunds:true` alakja sandboxból strukturálisan sosem figyelhető meg (jóváírást csak befejezett fizetésen lehet indítani, azt a tesztsuite emberi kattintás nélkül nem tudja előállítani); a `detailed:true` alakja megfigyelhető volt, de egy nem dokumentált mezőt (`currencyEnum`) is hozott, és a teljes modellezéséhez önálló válasz-oldali számla-/szállítási-cím-reprezentáció kellene. Mindkettő 2. fázisban, szándékosan, saját fixture-rel adható vissza — lásd 16. fejezet.
