...
Az `Urls` mind a négy címet kötelezővé teszi. A SimplePay a `start` kérésben kétféle formát fogad el: egy string `url` mezőt (egyetlen közös visszairányítási cím minden kimenetelre) vagy egy objektum `urls` mezőt a differenciált success/fail/cancel/timeout címekkel. Ez a csomag szándékosan mindig a differenciált formát küldi — a hívónak többet ér tudni, hogy a vásárló sikeresen fizetett, elutasították, megszakította vagy időtúllépés érte, mint a közös URL egyszerűsége —, ezért a payload mindig `urls` alatt megy ki, sosem `url` alatt. `url` maga **nem hibás vagy hiányzó kulcs**: ha objektumot csomagolunk alá string helyett, azt a SimplePay 5321-es hibakóddal ("Formátumhiba / érvénytelen JSON string") jogosan utasítja el — pontosan ez volt az app és e csomag korábbi terve közös hibája. **Nincs per-request IPN-cím mező** — sem `url`, sem `urls` alatt, semmilyen néven. A SimplePay v2 API-ban az IPN célcímet a kereskedői admin felület ("Technikai adatok" fül) adja meg, fiókszinten — ezt az osztályt nem terheli meg vele. (Ez a bekezdés eredetileg öt kötelező címet és egy `ipn`/`dn` mezőt írt elő, és tévesen úgy fogalmazott, mintha `url` maga lenne hibás kulcs; a Task 13 élő sandbox ellen futtatott kontraktus-tesztje és a hivatalos dokumentáció mezőlistája együtt pontosították — lásd a 2. fejezet 10. pontjának korrekcióját.)
...
**Miért `urls`, és nem `url`?** A SimplePay hivatalos dokumentációja mindkettőt elfogadja a `start` kérésben: egy string `url` mezőt (egyetlen közös visszairányítási cím sikerre, hibára, megszakításra és időtúllépésre egyaránt), vagy egy objektum `urls` mezőt, amiben ez a négy kimenetel külön címet kap. Ez a csomag **szándékosan mindig a differenciált `urls` formát küldi** — annak tudása, hogy a vásárló sikeresen fizetett, elutasították, megszakította vagy időtúllépés érte, önmagában eldönti, mit mutasson a bolt a visszatérő vásárlónak, ahelyett hogy egyetlen közös oldalon kellene ezt utólag kitalálni a `query()` hívásból. Aki a csomag kimenő payloadját a hivatalos mezőlistával veti össze, azért látja `url` helyett `urls`-t.
