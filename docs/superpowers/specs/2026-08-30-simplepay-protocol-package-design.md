...
**SDK-val megerősítve (Task 13, 2026-08-30).** A hivatalos SimplePay PHP SDK (2.1.5, 2026-06-27) `getSignature()`-je ugyanazt a két lépést végzi el, mint a mi `Signature::sign()`-unk: SHA-384 algoritmussal HMAC-et számol a nyers üzenet felett, majd az eredményt base64-kódolja — ez byte-azonos eljárás, egyetlen eltéréssel: **az SDK a kulcsot körbevágja (trim) a fehér térközöktől, mielőtt aláírásra használná, mind aláíráskor, mind a bejövő aláírás ellenőrzésekor.** A `Signature` korábban nem trimmelt — ezt a Task 13 ezen körében javítottuk (lásd lent), mert enélkül egy vezető/záró szóközzel bemásolt kulcs csendben más aláírást adott volna, mint amit a SimplePay (és a saját SDK-juk) számol, és minden hívás egy olyan hibával utasítódott volna el, ami sehol nem mutatott volna a valódi okra.
...
>
> **Redakció.** A rögzítés érzékeny mezőinek *értéke* egy nyilvánvalóan szintetikus jelölőre (`"[REDACTED]"`) cserélődik — a `SandboxTestCase` nyers rögzítője (`recordRaw()`) egy redakciós listát alkalmaz, mielőtt a fájlt kiírná, és legalább a `customer`, `customerEmail`, `invoice` mezőket és minden `salt` mezőt (a válasz tetején és a `transactions[]` elemein belül egyaránt) így jelöl meg, mielőtt a JSON lemezre kerülne — nem utólag, a rögzítés pillanatában. Szándékosan NEM törli a kulcsot: a nyers fixture egyetlen célja bizonyítani, mely mezőket küldi ténylegesen a SimplePay — egy törölt kulcs megkülönböztethetetlen lenne attól, hogy a mező sosem érkezett meg, pedig pont ez a két eset megkülönböztetése a fixture értelme. Egy beágyazott objektum (pl. `invoice`) a tagkulcsait megtartva, azok értékét cserélve redaktálódik — az alak (mely mezők léteznek) megmarad, csak a tartalom nem.
...
Minden sandbox teszt a kapott nyers választ kiírja `tests/Fixtures/sandbox/` alá, az érzékeny mezők értékét redaktálva (lásd fent) — a mezők maguk, a kulcsaikkal együtt, a válaszban maradnak.
...
**A kártya, a HUF és a HU nyelv az egyetlen kombináció, amit a Task 13 kontraktus-tesztjei ténylegesen elküldtek az élő SimplePay sandboxnak (koordinátori ruling, 2026-08-30, a `refunds:true`/`detailed:true` sorok pontosítása után felmerült kérdésre).** A `StartContractTest` a `StartRequest` alapértelmezéseit használja (`methods: [PaymentMethod::Card]`, `language: Language::Hu`), a teszt-kereskedő (`PUBLICTESTHUF`) pedig csak HUF-ot fogad. A `WIRE` fizetési mód, az `EUR`/`USD` pénznem és az `EN`/`DE` nyelv mindegyike helyesen szerializálódik és értelmeződik vissza — egységtesztekkel lefedve —, de a SimplePay sosem látta őket egyetlen kérésben sem: sem az nem ismert, hogy a szolgáltatás elfogadja-e, sem az, hogy az átutalásos fizetés speciális, a kártyás folyamattól eltérő, a beérkezésig nyitva maradó folyamatát a csomag helyesen kezelné-e — erről a csomag jelenleg semmit nem modellez. Egy enum-érték létezése és egységtesztelt szerializálása nem ugyanaz, mint egy élőben megfigyelt folyamat; ez a sor korábban tévesen egyenlővé tette a kettőt. A `Fizetőoldal nyelve` sor korábban csak a README-ben szerepelt, a specben nem — ez a korrekció a specbe is felvette, hogy a kettő ne váljon szét.
